...
import { Box, Typography, Stepper, Step, StepLabel, StepContent, Card, Stack, Chip, TextField, MenuItem } from '@mui/material';
...
const Sidebar = React.memo(({ intakeForm, onChange }: SidebarProps) => {
              ...
              <Step key={step} expanded active={index === activeStep} completed={completed}>
                ...
                <StepContent TransitionProps={{ unmountOnExit: false }}>
                  <Card sx={{ p: 2, mt: 1, background: '#ffffff', boxShadow: '0 8px 24px rgba(0,0,0,0.06)', borderRadius: 3 }}>
                    <Stack spacing={1.5}>{formStepFieldMap[step].map(renderField)}</Stack>
                  </Card>
                </StepContent>
              </Step>
            );
